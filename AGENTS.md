...
1. 이 프로젝트의 GitHub CLI와 push 인증은 사용자의 gh 를 그대로 사용한다.
...
5. 인증이 성공한 뒤에만 변경 범위 확인, 명시적 staging, commit, push를 수행한다.

## 12. 출입 장애의 무인 진단 원칙

1. 출입 무반응·인증 실패·Target 가용성 신고는 기존 진단 읽기 토큰과
   `scripts/read_diagnostics.py` / Backend 진단 API로 에이전트가 직접 조회한다.
   사용자에게 지원 보고서 복사, 로그 추출 또는 재현을 기본 대응으로 요청하지 않는다.
2. 자동 수집이 누락되면 수집 경로의 결함으로 취급한다. 마지막 정상 단계,
   자료의 실제 시각·신선도, 누락 범위를 먼저 확인하고 수집·보존·전송을 개선한다.
   오래된 snapshot이나 서버 수신 시각을 현재 상태·실제 발생 시각으로 대체하지 않는다.
3. 다른 가족의 세션을 신고자 성공으로 간주하지 않는다. 센서 체류 시간·거리,
   휴대폰 설정 또는 OS 제한을 증거 없이 원인으로 지목하지 않는다.
4. 검증은 화면 독립 수집, 서버 저장 확인, 장애 후 재전송과 사건 조회까지 포함한다.
   API/CI/OTA 성공과 실제 물리 출입 성공은 구분하며, 원인을 확정할 증거가 없으면
   확인된 단계와 증거 공백을 명시한다. 완료하지 않은 개선은 완료로 보고하지 않는다.
5. 자동 진단은 임의 개방·재부팅·OTA 재전송 권한을 부여하지 않는다. 실제 문을
   무인으로 반복 개방하지 않고, 가능한 회귀·장애 시험은 격리된 환경에서 수행한다.
