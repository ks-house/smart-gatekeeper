...
class RecoveryShellScreen extends StatefulWidget {
  const RecoveryShellScreen({super.key, required this.status, required this.missing});
  final String status;
...
class _RecoveryShellScreenState extends State<RecoveryShellScreen> {
  ...
  Future<void> _registerWake() async {
    try {
      ...
      if (mounted) setState(() => _updateMessage = 'Native wake is unavailable; use manual recovery.');
    }
  }
          ...
          child: ConstrainedBox(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
                ...
                child: Card(
                  child: Padding(
                    ...
                    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text('Setup status', style: theme.textTheme.titleLarge),
              ...
              Card(child: Padding(padding: const EdgeInsets.all(16), child: Column(
                ...
                children: [
                  Text('Native wake', style: theme.textTheme.titleMedium),
              ...
              Card(child: Padding(padding: const EdgeInsets.all(16), child: Column(
                crossAxisAlignment: CrossAxisAlignment.stretch,
                children: [
                  Text('Recovery capabilities', style: theme.textTheme.titleMedium),
                  const SizedBox(height: 10),
                  OutlinedButton.icon(
                    onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const SmartKeyControlScreen())),
                    icon: const Icon(Icons.lock_open),
                    label: const Text('Manual local / remote recovery'),
                  ),
                  OutlinedButton.icon(
                    onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const DebugScreen())),
                    icon: const Icon(Icons.monitor_heart),
                    label: const Text('Privacy-redacted diagnostics'),
                  ),
                  OutlinedButton.icon(
                  ...
                  TextButton.icon(onPressed: openAppSettings, icon: const Icon(Icons.settings), label: const Text('Open Android settings')),
                  if (_updateMessage != null) ...[const SizedBox(height: 8), Text(_updateMessage!, semanticsLabel: _updateMessage)],
                ],
              ))),
              const SizedBox(height: 12),
              Text(
