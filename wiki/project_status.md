...
> 관측 기준: exact-main `d9ecc87e04fc2b0e57cc892e549b02ddce26184a`의 connected ESP32-C6 Target `2.1.303+main.gd9ecc87`과 matching production-signed Fold7 앱은 action 1 `ARMED` 뒤 action 2 terminal open 및 Target relay-command ON→OFF를 완료한 마지막 connected core evidence다. MQTT host-gateway 변경은 feature main `7be876804c23d91caf252b92e2b859f81aee168a`로 병합됐고 final policy main `c569b15cbdcd7a166d8e2ded776bf8a54bc2414c`이 그 source baseline을 고정했다. Run `33252726976`은 immutable pull, DB health, migration `up 007`, API start와 새 stack loopback `/ready`를 통과해 MQTTS 복구를 증명했지만 NAS 내부에서 공인 `:4442` origin으로 돌아가는 DSM ingress probe가 실패했다. Partial stack은 볼륨 삭제와 DB rollback 없이 정리됐고 외부 `/live`·`/ready`는 레거시 중지 때문에 502다. 다음 source candidate는 HTTPS hostname/SNI/certificate 검증을 유지한 채 그 DSM listener의 transport만 NAS loopback으로 지정한다. WSL CH343 serial은 여전히 0 bytes지만 Android `SM-F966N`은 ADB authorized이고 앱 `1.0.0-gd9ecc87`이 실행 중이다. 레거시 복구, 신규 stack 배포 성공, 외부 readiness, backend-included `ARMED`→`OPENED`→relay ON/OFF 및 physical relay contact/load는 열린 Gate다.
>
