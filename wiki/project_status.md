...
> 관측 기준: exact-main `d9ecc87e04fc2b0e57cc892e549b02ddce26184a`의 connected ESP32-C6 Target `2.1.303+main.gd9ecc87`과 matching production-signed Fold7 앱은 action 1 `ARMED` 뒤 action 2 terminal open 및 Target relay-command ON→OFF를 완료한 마지막 connected core evidence다. 단일 routable bridge 변경은 feature main `dbafe9d4f803938d7570ef18769ef0925c6b0230`로 병합됐고 final policy main `6e8a1346f2e14d5ca3fd41b37d6975349e2d8be5`이 그 source baseline을 고정했다. Run `33251769358`은 immutable API/DB pull, 단일 `data` bridge, DB health, migration `up 007`, API start까지 통과했지만 시작 5.417초 뒤 MQTTS connect `TimeoutError`로 `/ready`가 실패했다. Partial stack은 볼륨 삭제와 DB rollback 없이 정리됐고, owner recovery 뒤 legacy `/live=200`, MQTT true가 복구됐다. 다음 source candidate는 public certificate hostname을 유지한 채 Synology API 컨테이너에서 그 hostname을 Docker `host-gateway`로 해석해 NAS public-IP hairpin만 우회한다. WSL CH343 serial은 여전히 0 bytes지만 Android `SM-F966N`은 ADB authorized이고 앱 `1.0.0-gd9ecc87`이 실행 중이다. 신규 stack 배포 성공, `mqtt=true`, backend-included `ARMED`→`OPENED`→relay ON/OFF 및 physical relay contact/load는 열린 Gate다.
>
