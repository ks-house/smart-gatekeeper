...
- Backend run `34012322823` deployed that exact source. NAS signed-release
  status, loopback/public readiness and independent strict-HTTPS `/ready` agree;
  all runtime checks pass, including MQTT and access-event collection.
- Target run `34012322944` published signed/encrypted
  `2.1.469+main.g6a45aec`. Independent signature, source, size and SHA-256
...
- Before this OTA, the old `2.1.462` image independently advanced boot 738 to
  ...
  retained coredump-valid flag alone is not attributed to the current reset.
