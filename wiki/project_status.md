...
> 관측 기준: exact-main `d9ecc87e04fc2b0e57cc892e549b02ddce26184a`의 connected ESP32-C6 Target `2.1.303+main.gd9ecc87`은 signed/encrypted periodic OTA 설치, pending-slot reboot, relay-OFF/Wi-Fi/MQTTS/signed ACL/GATT 복구와 application health-window `VALID` 표시를 완료했다. Fold7에는 matching production-signed `1.0.0-gd9ecc87` (`versionCode=24401`)을 replacement-install해 기존 앱 데이터와 AndroidKeyStore 자격을 보존했다. Native action 1 뒤 dashboard action 2는 terminal `문이 열렸습니다 (4612ms)`와 Target relay-command ON→OFF를 완료했고 정상 BLE 소유권 전환의 거짓 오류 배너도 제거됐다. 신규 NAS stack은 아직 `status=not-deployed`다. Exact backend run `33246998513`은 새 DB health와 migration `up 007`, 새 API process startup까지 통과했지만 MQTT subscriber/ACL publish의 `ConnectionRefusedError` 때문에 loopback `/ready`가 503을 유지했다. Owner readback으로 legacy endpoint `tworimpa.synology.me:4883`과 신규 runtime의 `MQTT_PORT` 누락 및 Compose `8883` 고정 불일치가 확정됐다. Wrapper는 partial project만 볼륨 삭제 없이 정리했고 retained legacy DB/API는 다시 `running`이다. 현재 source candidate는 exact legacy TLS port를 보존하고 plaintext `1883`을 거부하지만 아직 정책/CI/배포 증거가 아니다. Physical relay contact/load, actual door motion, sensor threshold와 반복/OEM 분포는 열린 Gate다.
>
