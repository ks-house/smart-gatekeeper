...
The Target implementation now verifies Ed25519 manifests, downloads only over CA-verified HTTPS, writes the inactive OTA partition, checks exact size/SHA-256/image validity, selects the candidate only after verification, and uses pending-verify health marking or automatic rollback. Periodic HTTPS and authenticated local WPA2/Basic recovery are independent of MQTT, while signed `ota_check` remains an optional trigger. Protocol overlap 1..2, a durable version floor, the previously bootable slot, and manual local recovery preserve N/N-1 and rollback paths.
