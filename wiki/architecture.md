...
##### 2026-09-10 HA manual-open incident: read-only evidence

**Correction deployed:** PR405/main8e4bd56, Backend run34379245548, external exact-build ready/all12 checks/fresh Target confirmed01:55:55 KST and again around01:56:45. HA bridge now generates sessions with `uuid.uuid4().hex`, separately from its unchanged128-bit random nonce factory. Compact UUIDv4 version/variant validation occurs before request/rate-limit reservation. Wrong-format sessions cannot be signed/published by this bridge. Unit tests cover generation/signature preservation for manual/OTA/reboot, rejected field-session/variant/case/type inputs and unconsumed reservation. A cross-language test compiles the actual Target tracker and rejects the captured failing ID while accepting32 new bridge-generated sessions.173 focused tests pass including Backend boot/command, discovery migration, Target security/OTA contract and hardwareless production core. Target/app unchanged; no physical door test claimed. Durable HA result storage remains a separate pending enhancement. NAS apply/status records match; bridge reconnects after9.37s API replacement gap, Target remains491/boot816/IDLE/relay OFF.

**Root cause confirmed at01:41:35 KST:** owner's observed press reached HA ingress at01:41:35.641 (`PRESS`, non-retained, non-duplicate). Backend published signed `manual_remote` at01:41:35.681, session `4928b7ca412dd60e8b614b0e22394f08`, current boot816. Target ACK at01:41:35.855 returned13; bridge correlated `target_rejected` at01:41:35.895. Code13 is `kEffectRejected`, not a transport or signature failure. HA bridge generates random hex via `secrets.token_hex(16)`, whereas Target `SignedCommandAccessTracker::normalizeUuid4` requires UUID version4/variant bits. The received session has `d` at compact index12 rather than `4`; it deterministically fails tracker preflight. Mobile default signer uses `uuid.uuid4().hex`. This matches the reported path-specific failure.

Local executable uses the real `src/TargetCommandSecurity.cpp`: exact received session rejects, a control differing only in the UUID version nibble accepts, enum result13 confirmed. No physical effect is invoked. Corrective implementation should generate and validate UUIDv4 HA session IDs and add cross-component regression coverage; preserve cryptographic nonce generation, signature/replay/boot checks and Target event identity contract. No runtime fix or deployment has occurred in this diagnostic turn. Earlier uncertainty below records the investigation sequence and is superseded by this captured request/ACK evidence.

HA transport is `HA button -> MQTT bridge request -> Backend signer -> signed MQTT Target command -> Target ACK/result`. It is not HA-to-Backend HTTP. Direct MQTT subscription can observe every live message on those topics; inability to reconstruct older non-retained messages is a separate limitation. The01:37:50–01:38:50 KST passive five-topic subscription succeeded and received57 verified statuses but no request/result/command/ACK. No owner click was confirmed in that minute; do not interpret silence as a failed HA publish. Observer stopped at the end of that window.

Subsequently the owner confirms a01:37 press, exact seconds unknown. It may predate the01:37:50 subscription. A fresh01:36–01:39 API query returns no access events and unchanged boot816/IDLE/OFF in health rows3000–3005. Thus no additional successful actuation is recorded, while HA publication versus Backend rejection remains unresolved. A new bounded passive observer runs01:40:11–01:50:11 KST; this is not persistent deployed collection.

- Owner reports mobile-button opening works but HA opening does not. At01:32–01:34 KST, Backend106a2d2 `/ready` is healthy/fresh; verified history row2995 received01:33:32.673 reports boot816 IDLE/relay OFF, with unsigned firmware491/uptime1198/OTA VALID advisory. No new reset was observed in this query window.
- Access API returns row9093 received01:28:24.142 KST: `ACCESS_SIGNED_MANUAL_COMPLETED`, `ACCESS_GRANTED`, `mqtt_manual_remote`/`signed_mqtt`, boot816. This verifies a signed manual command completed, not independent physical movement or attribution to a particular HA/mobile button. Owner supplies the mobile physical-success observation.
- Current mobile `RemoteManualOpenService` uses authenticated Backend HTTP followed by signed MQTT, not necessarily the historical local-GATT manual path described above. HA uses its separate bridge ingress, then the same signer/publisher and Target manual action.
- Passive strict-TLS MQTT observation confirms retained `open_gate` discovery points to `gatekeeper/v1/ha-bridge/c0feffe6ebac/request/open_gate`, exact `PRESS`, QoS1/non-retained, and live bridge availability is online. This verifies published discovery, not HA's internally loaded entity or delivery of the earlier click. Presence of the button is not direct proof of the current runtime manual-remote environment flag.
- `publish_bridge_result` emits rejection/publish/ACK results non-retained, with no durable bridge-result write in the inspected handler. Rejected ingress has no session identifier. Existing read APIs therefore cannot reconstruct the earlier HA rejection/ACK; no result arrived during the12-second passive sample. Root cause remains unconfirmed, not assumed disabled permissions, old firmware, sensor proximity or brownout.
- Required diagnostic improvement: bounded persistent HA request/rejection/publish/Target-ACK records with a correlation ID even before signing, read-token API exposure, and HA result display. Preserve command authorization and non-retained commands. No door command, reboot, OTA, HA configuration mutation or runtime implementation was performed for this diagnosis.
- Owner's subsequent HA screenshots show button presses01:27:18,01:27:43,01:29:00, but relay ON01:28:13/OFF01:28:14 only. Exact01:27–01:30 KST API read corroborates rows2983/2984 at01:28:13.004 RELAY_HOLD/ON and01:28:14.135 COOLDOWN/OFF, then2985 at01:28:24.041 IDLE. Access row9093 receipt01:28:24.142 is not the relay-on timestamp. Rows2980–2988 retain boot816 and increasing uptime. The01:29 click is about36 seconds after return to IDLE; the earlier relay cooldown does not explain it. No matching additional successful Target access event is stored in the queried interval. HA records the presses, but Backend ingress/delivery/rejection remains unobserved; do not reinterpret the01:28 relay pulse as a delayed successful HA click without source correlation.
