...
## [2026-08-02] fix | Trusted workflow policy required-check deadlock 해제 (paths 필터 제거)

- `.github/workflows/trusted_workflow_policy.yml`에서 `paths` 필터를 제거하여 docs-only PR을 포함한 모든 main 대상 PR에서 `Verify protected files against trusted base policy` 검사가 실행되도록 수정
