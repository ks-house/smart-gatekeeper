...
## [2026-09-08] test | Deploy standalone access-history API and verify existing token readback

- Owner requested deployment. PR #390 passed checks and merged to exact main 3c08e6b8ce60fd693103df868715215e8fbfe2f2. Read-only NAS preflight 34211540652 succeeded; Backend run 34211698089 passed real-MariaDB/security/operations/image checks and production deployment at 18:50 KST. Forced apply/status evidence matched; API digest 6955b04f9275f2d20a3594b5376eb9bcf5bbc8b754991d241da1e6bd7a093f47.
- Independent HTTPS readiness returns exact deployed source, all checks true and fresh verified Target status. Existing local token reads today's access history, two disjoint cursor pages and combined Target/session/boot/event filters. Latest inspected receipt is 16:11:22.784 KST, ACCESS_SESSION_COMPLETED; no physical-door inference.
- Existing report list remains readable; missing/wrong tokens and diagnostic-token use on admin access history return 401. Authenticated reads used normal system DNS and verified TLS. No token/digest/wrapper change, migration, mobile/Target publication or device command. Updated rollout guide while preserving unrelated incident-document edits.

## [2026-09-08] test | Reassess silent entry failure with direct history and reproduce audit loss

- Interpreted owner arrival as September 8 after 21:50 KST. Existing read token returned two boot-782 sessions in the evening receipt window: connection/challenge then disconnect after 1,200 ms, and verified proof/ARM then ARM_TIMEOUT after 60,013 ms. Sixty seconds of the latter events arrived within 815 ms; receipt time is not physical occurrence time. Owner-phone attribution and precise sensor subcause remain unproven.
- Paginated today's history to exhaustion: 59 verified rows in 18 sessions, with terminal reasons 9 ARM_TIMEOUT, 4 GATT_DISCONNECTED, 4 ACCESS_GRANTED and 1 OTA_BUSY. These are not 18 independent owner arrivals. Current exact-source Backend readiness and Target freshness pass; newest mobile report is still September 7 15:50:48, so today's phone wake/worker trace is absent.
- Parallel read-only audits found UI-lifecycle-dependent diagnostic upload, missing skipped-dispatch decisions, unreconciled uncertain-proof blocking, sensor-clearance indefinite-block risk, ARMED-wide MQTT deferral and missing immutable measurement/boot history. None is assigned as this owner's exact physical failure without correlated evidence.
- Actual OfflineEventQueue.cpp host reproduction with always-success storage accepted all 14 representative normal-access rows but restored only canonical RELAY_OFF and SESSION_COMPLETED. A nine-row ARM_TIMEOUT flow restored sequences 6–9 while losing GATT_CONNECTED sequence 5. Eight-slot mixed-event checkpoint overflow is a reproduced evidence-retention defect even without network failure; it is not a reproduction of the physical entry failure.
- Added linked reliability recovery proposal: native consent-bound durable upload, canonical retention and Backend commit ACK, frozen sensor/session summaries, independent incident/health history, explicit sensor/reauth states, single-owner transport isolation, bounded actual-Android test bench and natural-use acceptance. Independent RF/power/door observation and a cryptographic key alternative remain explicit owner decisions if needed. No runtime code/configuration, deployment, Target command or hardware action; preserved earlier documentation edits.

## [2026-09-08] code | Start approved screen-independent reliability recovery

- Owner approved implementation and requires Agent-direct diagnosis without requesting copied reports. Added project incident-response rules: existing diagnostic API first, stale/missing data is a collection defect, no unsupported user/OS attribution, and no automatic actuator authority.
- Started coordinated mobile native upload, firmware canonical retention/sensor journal, and Backend incident/history implementation on codex/hands-free-reliability-recovery. Earlier incident/deployment documentation edits are preserved.
- Added isolated event/sensor DB-commit receipt authentication helpers with separate domains and exact identity binding; independent Python versus firmware HMAC vector and tamper/wrong-event tests pass. Added immutable telemetry worker and blocked-socket host test; 1,000 control ticks, no dual owner and heap/task/watchdog failure paths pass. Transport integration, full regressions and deployment are still pending; no physical result claimed.

## [2026-09-08] fix | Complete autonomous diagnostic capture and bounded reliability recovery candidate

- Native opt-in outbox now operates independently of Flutter screens, records skipped dispatch/worker/lifecycle/OS evidence and retires only exact accepted reports. Clear, credential/upload authority changes and logout invalidate generations; pre-proof orphan recovery queries actual WorkManager state and never replays uncertain proof. Kotlin-generated report passed strict Backend validation and authenticated API exact-ACK test. Native 93 and Flutter 110 tests passed with changed Dart analysis clean; actual phone install/background execution remain unverified.
- Separated best-effort legacy from canonical audit, retained authenticated records until exact domain-separated DB commit receipt, and kept unsigned V1 compatibility retirement. Added four-window sensor capture and four-record two-slot NVS journal with corruption/drop/persistence counters; no invalid sensor sample grants entry. Main-loop sensor/relay control precedes an exclusively leased immutable-status worker during ARMED; inbound MQTT commands and audit flushing remain IDLE work. Queue retention, torn storage, sensor MAC/uint32 rollover, receipt rejection and blocked fake socket regressions pass.
- Backend schema 016 adds append-only sampled health/sensor history plus read-only incidents/health-history on the existing token. Signed evidence and unsigned advisory remain separate; mobile-only skips/failures and sensor-only observations survive missing Target event chains. Backend/PC 264 tests passed (three separately exercised integration skips); actual MariaDB schema/API SQL and production image repeat-up/rollback preservation tests passed. Migration SHA is 66c40f335fa9dfd3f857005b755f57e91c52b3fdcd5c36664a4eebbcda3029e5.
- Production-enabled local firmware build passed with RAM 93112B and Flash 1842052B; OTA contract and protocol 16 tests pass. Existing hardwareless profile produced ambiguous duplicate feature defines, so enabled-production validation used the actual personal-production profile. No physical door/RF/phone result inferred. Root test failures are limited to expected protected-byte admission before the separate two-input policy transition; full final regression and deployment readback follow.
...
## [2026-09-09] test | Verify reliability release publication and real Backend evidence collection

- PR #391 policy admission and PR #392 feature merged to exact main 81fbc6dc9b9286b2d4375afeda9129566789ad2a. Final root 391 tests pass (one PowerShell skip), native 94 and Flutter 110 pass; all required PR checks pass. Backend run 34240363175 deployed schema 016 at September 8 23:54 KST; independent readiness matches source with all 12 checks true and Target fresh. Existing token observes real health rows increasing about every 30 seconds and historical failed incidents; old report/access reads and no-token/wrong-token 401 checks pass.
- Target run 34240363227 published 2.1.480+main.g81fbc6d at 23:51 KST, with encrypted/signature verification and exact HTTPS manifest/artifact readback. Mobile run 34240363172 published 1.0.0-g81fbc6d/build44401 at September 9 00:02:20 KST; signing and primary/fallback atomic publication plus exact HTTPS metadata/APK readbacks pass. APK SHA-256 d1efd6dfeb68b4885b1f229c3ccb665b72d283b8edcf262f46f6df78494138e4, 55,659,673 bytes.
- Actual Target health still reports old 2.1.469+main.g6a45aec, same verified boot784 with increasing uptime. Repeated same-boot BROWNOUT is not evidence of another reset. Today’s sole OTA_BUSY event predates this publication. No manual OTA trigger, reboot, relay command or physical test was performed; publication, device installation and physical success remain separate. New APK requires user installation and one initial app open to migrate existing native-upload consent/configuration, not report copying.
- Registered hourly read-only thread follow-up SGK 출입 신뢰성 후속 진단. It compares existing API evidence, reports only meaningful changes and never issues device/control/deployment commands. PC/app availability is required for the Agent schedule, separate from NAS collection. Preserved unrelated incident-analysis edits without staging them.

## [2026-09-09] fix | Close diagnostic cache-header and discarded boot-advisory gaps

- Live HTTPS verification exposed global middleware overwriting router no-store with no-cache. Corrected the exact diagnostic namespace for success and all handled/unexpected error responses; actual main.app tests cover 200/401/422/404/405/429/503/500 without raw exception disclosure or unrelated cache-policy expansion.
- Added bounded, closed-field boot advisory capture for already emitted Target /boot messages, including explicitly marked retained replay. Only exact configured topics and subsequent matching MAC-verified status identities can attach unsigned advisory to existing sampled health history. No boot authority, freshness, command, migration, wrapper or token change. Added GATT/network/heap/previous-access and sensor-clearance projections that the prior read API discarded. Source review preserves unsigned-versus-authenticated and receipt-versus-generation-time boundaries; final regression and production readback follow.
- Final Backend/PC suite passes 274 tests (three optional integration skips); root 391 tests pass (one PowerShell skip). Actual app-stack cache errors, boot identity/redaction/retained subscriber tests pass, OTA contract and whitespace/link-target checks pass. The changes are frozen for a Backend-only hotfix rollout; mobile and Target publication sources remain 81fbc6d.

## [2026-09-09] test | Deploy diagnostic readback corrections and confirm actual boot observation

- PR #393 passed checks and merged to exact main a105539273f9eb2aeaf0362d17bcc967336d85a7. Backend run 34243125046 passed production checks and deployed 00:14:44–00:16:16 KST. Independent readiness matches exact source/all 12 checks true/Target fresh. Four diagnostic endpoints each return exact no-store for 200, no-token 401, wrong-token 401 and invalid-query 422; 16 live read cases pass without production fixture writes.
- Existing health rows 1 and 36 preserve original data. New row46 received00:17:10.672 contains matching boot784 UNSIGNED/MQTT_BOOT_ADVISORY, retained=true, receipt00:16:07.306 and generation time NOT_OBSERVED. Forbidden identifiers/raw fields remain absent; no boot data is promoted into signed core. Actual fields report planned_restart none, previous action https_date_clock_trusted, previous IDLE/relay OFF and previous uptime1977020ms. Sameboot BROWNOUT9 is not a new reset or definitive cause of the earlier arrival failure.
- Actual Target remains 2.1.469+main.g6a45aec, boot784; new2.1.480 installation is unobserved. Current unsigned BLE expected/active flags are true with no active connection; currentboot GATT/auth/sensor/relay counters are zero. No RF reception or physical result is inferred. Updated source-of-truth release/API documentation and retained hourly read-only follow-up; no further APK/firmware publication, device commands, wrapper or token changes.

## [2026-09-09] test | Read-only post-release checkpoint through 01:00 KST

- Existing diagnostic token queried the fixed receipt window 2026-09-08T15:21:20.012Z through 2026-09-08T16:00:20Z (September 9 00:21:20.012–01:00:20 KST). Health history returned 77 rows, IDs 54–130, with the cursor exhausted in one page; latest receipt 01:00:19.090 KST, maximum adjacent sample gap 33.690 seconds. All verified cores remain boot784/a629abaae2dbcea8727cf16ded870f94, IDLE and relay commanded OFF. Independent readiness remains exact Backend a105539273f9eb2aeaf0362d17bcc967336d85a7, all 12 checks true and Target fresh.
- Unsigned advisory remains firmware 2.1.469+main.g6a45aec, uptime 5835→8174 seconds, unchanged MQTT connection/outage/error counters, BLE expected/active true and no new GATT/auth/sensor/relay counts. Repeated same-boot BROWNOUT is not another reset. No new installation, reboot or communication-failure evidence was observed in this window; these are not independent RF or physical-door observations.
- Access events and Target incidents each returned zero rows and exhausted their cursors in one page; mobile incidents and sensor observations were also empty. Event/mobile-incident/sensor/health truncation flags are false, but mobile_reports_truncated=true for the bounded historical report source. Latest report is still row143, received September 8 23:41:17.088 KST, generated 23:41:16.591858, app44201/g00bee34 without native.runtime. Queried reports are STALE_REPORT with no in-window session/lifecycle evidence; this is not evidence of no arrival, successful entry, or new APK44401 execution.
- No new actionable change relative to the previous checkpoint. Kept the existing read-only follow-up; no device commands, runtime changes, deployment, commit/push, or request for owner-copied logs. Preserved unrelated continuous-presence documentation edits. Whitespace validation follows.

## [2026-09-09] test | Observe new Target firmware execution and recurring resets

- Read-only heartbeat covered fixed UTC 2026-09-08T16:00:20Z–16:58:53.823Z (September 9 01:00:20–01:58:53.823 KST). Health IDs131–242 total112, two pages exhausted; all cores verified/IDLE/relay commanded OFF. Access and incident cursors exhausted with zero access/Target/mobile incidents or sensor summaries. Incident-internal health100-row truncation is covered by the independent112-row read; historical mobile reports remain truncated/stale, latest row143/app44201 unchanged. Empty access evidence does not establish no arrival or successful physical entry. Readiness remains exacta105539/all12 checks true/Target fresh at read time.
- Verified boot transitions distinguish four new restarts: boot785 first received01:17:00.325 on469; boot786 first received01:19:36.543 on newly published480; boot787 first received01:50:36.871 and boot788 first received01:57:27.686 both on480. These are first status receipt times, not exact reset times. Boot786 supplied61 samples over30m47.991s before the next reset. Largest receipt gap125.359s surrounds the469→480 transition. New firmware execution is now observed, but direct slot-valid/health-confirmation telemetry and physical access remain unobserved.
- Exact-identity UNSIGNED boot advisory reports SOFTWARE/ota_pending_verify for786, and BROWNOUT/code9 with planned_restart none for785/787/788; new observations have retained=false, generation_time NOT_OBSERVED. Later two previous-action breadcrumbs are https_date_clock_trusted with previous uptimes1861667ms and410795ms; these are last recorded actions, not causal fault locations. Current source reads esp_reset_reason() directly. [Espressif C6 documentation](https://docs.espressif.com/projects/esp-idf/en/v5.5.4/esp32c6/api-guides/fatal-errors.html#brownout) identifies brownout as the low-supply detector; the unsigned report is a power-integrity lead, not voltage measurement or proof of a defective supply module. A similar reset occurred before480 as well as twice after it, so neither OTA-only causation nor successful availability recovery is established.
- New480 advisory minimum-ever heap reached1740B on786 and4068B on787; this is a separate memory-headroom concern, not proof that memory caused BROWNOUT. OTA source still requires30s continuous safe/network/heap checks within120s and explicit mark-valid; sampled state cannot replace that result. Proposed follow-up: authenticated durable OTA completion/reset evidence and bounded stage-specific heap measurements; investigate supply integrity without disabling brownout/rollback protection or prescribing an unverified mains module.
- Source inspection also identifies a remaining idle-sensor evidence gap: main.cpp845 calls readDistanceCmRaw without recording duration/counters, while UltrasonicSensor.cpp72–78 updates counters only in the ARMED filtered reader. No-echo and invalid/out-of-range readings collapse to999. Thus observed IDLE sensor_samples0/clearance UNKNOWN does not distinguish sensor polling from no echo or invalid returns. Proposed fix is separate bounded idle sample/result counters and authenticated health projection, preserving existing no-echo denial and session statistics; not implemented in this read-only heartbeat. No control/OTA/reboot, code/configuration/deployment changes, commit/push or user-log requests. Existing edits preserved; whitespace checked.

## [2026-09-09] test | Quiet read-only checkpoint through 03:00 KST

- Fixed UTC window2026-09-08T16:58:53.823Z–17:59:54.803Z: health120 rows/IDs243–362, two pages exhausted, latest receipt02:59:48.677 KST, maximum adjacent gap31.353s. All verified cores remain boot788/d39183b5fd5548237ea7fadc6425616c, IDLE/relay commanded OFF; advisory480 uptime99→3749s. No new reboot, firmware change or observed status gap. Readiness remains exacta105539/all12 checks true/Target fresh. Same-boot BROWNOUT is not a new event.
- Access/Target incidents/mobile incidents/sensor summaries each zero; access and incident cursors exhausted. Latest mobile row143 unchanged; no new detail fetched. Event/mobile-incident/sensor truncation=false; historical mobile_reports and incident-internal health truncation=true, with the latter covered by the complete independent120-row health query. Missing phone/arrival/physical-door evidence remains unchanged, not proof of successful or absent access.
- Sensor advisory samples are FAULT93/UNKNOWN21/CLEAR6. Read-only comparison of the previous window found FAULT61/UNKNOWN16/CLEAR1 among78 new480 rows, first FAULT already01:20:07.046 KST/row166/boot786. Thus this is not a newly started fault. PassageRearmPolicy.h classifies ten consecutive absent/invalid measurements as FAULT; the previously recorded raw-reader counter gap prevents separating no-echo from invalid/out-of-range input. No new hardware-cause attribution or duplicate stale/fault alert. App/Target install-health and physical acceptance boundaries are unchanged.
- No new actionable change; quiet completion. Only this observation appended; preserved existing edits, no device command, code/configuration/deployment change, commit/push or user-log request. Whitespace checked.

## [2026-09-09] test | Read-only checkpoint finds reduced contiguous heap after MQTT reconnect

- Fixed UTC window2026-09-08T17:59:54.803Z–19:03:55.754Z (02:59:54.803–04:03:55.754 KST): health125 rows/IDs363–487, two pages exhausted, latest receipt04:03:50.766 KST, maximum adjacent gap36.681s. All verified cores remain boot788/d39183b5fd5548237ea7fadc6425616c, IDLE/relay commanded OFF; unsigned advisory firmware480/uptime3779→7591s. No new boot or firmware transition. Readiness remains exacta105539/all12 checks true/Target fresh at read time; repeated same-boot BROWNOUT is not another reset.
- A new unsigned advisory edge first received03:55:08.031 KST/row470 shows MQTT connect count/attempts1→2, failures0 and Wi-Fi outages0. At that edge largest_free_block falls34804→18420B and remains18420 through18 samples ending04:03:50.766 (8m42.735s); total free heap remains about64184B and minimum-ever4252B is unchanged. This is reduced contiguous allocation headroom temporally associated with reconnect, not proof of a leak, allocation failure, power fault or entry-failure cause. Current OtaManager.cpp62,928–997 requires32KiB largest block only during the pending-image health window: these later runtime samples do not establish failed OTA health, automatic rollback, or inability to perform a future OTA. Reconnect/headroom regression coverage is a follow-up candidate, not implemented by this read-only check.
- Independent access/Target incident queries returned zero rows with cursors exhausted; mobile incidents, sensor summaries and in-window mobile session/lifecycle evidence are also empty. Event/mobile-incident/sensor truncation=false; historical mobile reports and incident-internal health truncation=true, with health coverage supplied by the complete independent125-row query. Latest mobile row143/app44201 remains unchanged; no new detail fetched or44401 execution evidence. Empty records do not prove no arrival or successful entry. Existing sensor FAULT109/UNKNOWN14/CLEAR2 samples and the raw-reader evidence gap are not new alerts.
- Only this bounded observation was appended. No device/control/OTA/reboot request, code/configuration/deployment change, commit/push or owner-log request; existing documentation edits preserved. The newly sustained contiguous-headroom decrease is distinguished from unchanged historical alerts. Whitespace validation follows.

## [2026-09-09] test | Quiet read-only checkpoint through 05:04 KST

- Fixed UTC2026-09-08T19:03:55.754Z–20:03:56.592Z: health117 rows/IDs488–604, two pages exhausted, latest receipt05:03:42.179 KST, maximum adjacent gap31.748s. All verified cores remain boot788/d39183b5fd5548237ea7fadc6425616c, IDLE/relay commanded OFF; advisory480/uptime7621→11182s. Readiness remains exacta105539/all12 checks true/Target fresh. No new boot, version change or observed status gap.
- MQTT count2, minimum-ever heap4252B and largest block18420B remain unchanged; latest free heap64040B. Existing sensor FAULT109/UNKNOWN8, same-boot BROWNOUT and prior contiguous-headroom concern are not new failures or proof of hardware/entry failure. OTA health confirmation and independent physical evidence remain unobserved.
- Access/Target incidents each zero with cursors exhausted; mobile incidents/sensor summaries/in-window mobile session/lifecycle evidence also zero. Event/mobile-incident/sensor truncation=false; historical mobile reports and incident-internal health truncation=true, with health covered by the independent117-row query. Latest mobile row143 unchanged; no new detail fetched or44401 execution evidence. No new actionable change; quiet completion, no repeated stale alert. Only this observation appended; existing edits preserved, no device commands, code/configuration/deployment changes, commit/push or owner-log requests. Whitespace checked.

## [2026-09-09] test | Observe complete new-firmware access evidence and three further resets

- Fixed UTC2026-09-08T20:03:56.592Z–21:03:57.765Z: health120 rows/IDs605–724, two pages exhausted, latest receipt06:03:30.422 KST, max adjacent gap31.938s. Verified states IDLE117/ARMED1/RELAY_HOLD1/COOLDOWN1. Readiness exacta105539/all12 checks true/Target fresh at read time. Firmware advisory480 throughout; no new publication or direct OTA health-confirmation evidence.
- New verified boots789/790/791 first received05:17:26.929/05:59:18.749/06:02:29.638 KST, not exact reset times. Each matching non-retained UNSIGNED boot advisory reports BROWNOUT9/planned_restart none. Previous actions are mqtt_connect_worker_adopted/https_date_clock_trusted/relay_off, each previous IDLE/relay OFF. Boot791 previous uptime190086ms is29.066s after its previous COMPLETED access breadcrumb161020ms; this does not establish a relay-edge-induced reset. [Espressif brownout documentation](https://docs.espressif.com/projects/esp-idf/en/v5.5.4/esp32c6/api-guides/fatal-errors.html#brownout) supports a supply-integrity lead, not identification of a failed power component from unsigned data. Boot788 MQTT count2→6, failures0/Wi-Fi outages0; its largest block remains18420B. Boot790 minimum-ever heap1780B is a separate concern, not a demonstrated reset cause.
- First observed480 full canonical chain: access IDs521–528, boot790/session2dd3034e-b388-4052-9095-17342578b0ac, verified sequence1–8 through ACCESS_GRANTED/COMPLETE, cursor exhausted. Original monotonic GATT→ARM260ms, ARM→sensor14096ms, sensor→relay2ms, relay pulse1022ms, overall15380ms. Receipt06:02:04.609–06:02:09.218 is delayed delivery, not occurrence time. Signed status independently shows ARMED→relay commanded ON/pin0→COOLDOWN→IDLE with matching terminalseq8/phase31. No door contact or mobile identity match: FLOW_REPORTED_COMPLETE is not physical entry or owner-attributed success.
- First independently MAC-verified sensor summary row1, received06:01:52.237, matches that boot/session/terminalseq8:108 samples,102 timeouts,6 valid,0 invalid,3 in range, min387/max2187/last raw and median408mm, threshold800mm. Source counts timeout only when pulseIn returns duration0. The14.096s window is sensor waiting after ARM, not proven time spent physically at the door; arrival and per-sample chronology are absent. Full8-event DB preservation and terminal sensor-summary delivery now have real Target evidence. RAM outbox4→0 alone cannot prove all commit ACKs reached Target because migration to NVS also drains that RAM queue.
- Incident1, mobile incidents0; access/mobile-incident/sensor truncation=false, historical mobile reports and incident-health truncation=true (health covered independently). Latest mobile143/app44201 unchanged, no44401 execution evidence. Proposed follow-up remains signed durable reset/OTA completion plus stage-specific heap metrics, bounded sensor timeout-run/idle measurements and isolated MQTT reconnect/memory regression; no guessed sensor-distance blame, timeout-as-clear bypass or unverified power-module prescription. Only this observation appended; no device command, code/configuration/deployment change, commit/push or owner-log request. Existing edits preserved; whitespace checked.

## [2026-09-09] test | Quiet checkpoint through 07:05 KST with no further boot transition

- Fixed UTC2026-09-08T21:03:57.765Z–22:04:58.819Z: health120 rows/IDs725–844, two pages exhausted, latest receipt07:04:55.450 KST, max adjacent gap34.288s. All verified cores remain boot791/9e4e4c73d5289b00bc765823f858a886, IDLE/relay commanded OFF; advisory480/uptime99→3753s. Readiness exacta105539/all12 checks true/Target fresh. No new reboot, version transition or observed status gap.
- The already reported reconnect/headroom pattern recurs: unsigned MQTT count1→6, failures0/Wi-Fi outages0; largest block59380→29172B at06:10:38.894, briefly35828, then25588 from06:21:27.385 through the last sample. Latest free heap63884B/minimum-ever4132B; no new GATT/proof/sensor/relay counts. Repeated same-boot BROWNOUT and refreshed boot-message receipt are not new resets. Existing sensor FAULT98/UNKNOWN19/CLEAR3 and headroom concerns remain open, without a newly observed failure consequence or causal attribution.
- Access/Target incidents each0 with cursors exhausted; mobile incidents/sensor summaries/in-window mobile session/lifecycle evidence also0. Event/mobile-incident/sensor truncation=false; historical mobile reports and incident-health truncation=true, health covered independently. Latest mobile143 unchanged, no new detail fetched or44401 execution evidence. No new actionable class of failure/install/physical result; quiet completion without repeating prior alerts. Only this observation appended; existing edits preserved, no device/code/configuration/deployment changes, commit/push or owner-log requests. Whitespace checked.

## [2026-09-09] test | Observe native mobile uploads and three additional Target resets

- Fixed UTC2026-09-08T22:04:58.819Z–23:05:29.793Z (07:04:58.819–08:05:29.793 KST): health118 rows/IDs845–962, two pages exhausted, last receipt08:05:04.667 KST, max adjacent gap34.727s. All verified cores IDLE/relay commanded OFF; readiness exacta105539/all12 checks true/Target fresh. New verified boots792/793/794 first received07:12:27.728/07:32:56.783/07:45:33.207 KST, not exact reset times. Matching non-retained UNSIGNED boot advisories each report BROWNOUT9/planned_restart none/previous action mqtt_connect_worker_adopted. Firmware advisory480 throughout. These observations confirm further boot transitions, not a voltage measurement or proof that MQTT caused the reset.
- First new-app execution and native automatic-upload evidence: new reports144–151 include old44201 report144 and seven44401/g81fbc6d reports145–151. First new-app receipt07:44:00.162 KST (capture07:43:59.824), latest151 receipt08:05:00.261 (419ms after capture). The same native process emits HEARTBEAT reports and later reports advance the previous upload ACCEPTED/last-success fields. This establishes actual native collection and repeated Backend receipt, not screen-off physical-entry acceptance. Report151 is the new comparison baseline; its36 stored sessions are historical, not new attempts.
- Access/Target incidents/mobile incidents/sensor summaries each0 in the fixed window; access and Target incident cursors exhausted. New mobile reports contain no in-window session or wake evidence. Historical mobile-report and incident-internal health caps are truncated, with complete health coverage supplied independently above. Boot792 advisory counts one bare GATT connection and zero verified proofs; the following boot records CONNECTION_ACCEPTED at previous uptime19281ms versus reset breadcrumb1228321ms. No reporter identity, new authentication result, or reset-synchronous connection cause is inferred.
- Reconnect/contiguous-headroom and sensor-observation concerns remain open: minimum-ever heap3676/3396/3980B in the three new boots; latest boot794 uptime1183s/free63852B/largest20980B/MQTT connects5/failures0/Wi-Fi outages0. Sensor advisory FAULT96/UNKNOWN18/CLEAR3/OCCUPIED1 does not resolve the existing idle raw-reader counter gap. Only this observation appended; no device/control/OTA/reboot, code/configuration/deployment change, commit/push or owner-log request. Existing documentation edits preserved; whitespace validation follows.

## [2026-09-09] test | Correlate screen-off authentication with sensor timeout and misleading busy reason

- Fixed UTC2026-09-08T23:05:29.793Z–2026-09-09T00:06:30.815Z: health125 rows/IDs963–1087, two pages exhausted, latest receipt09:06:11.572 KST, max adjacent gap35.745s. Readiness exacta105539/all12 checks true/Target fresh. Five new verified boots795–799 first received08:06:23.833/08:13:59.025/08:38:40.791/08:53:45.282/08:58:31.630 KST. Matching non-retained UNSIGNED advisories report480/BROWNOUT9/planned_restart none, previous IDLE/relay OFF; previous action is mqtt_connect_worker_adopted except797 ota_manifest_get. First receipt is not reset time and breadcrumbs are not causal proof. Boot796 minimum-ever heap27236B versus7953444/7973968/7984044 does not establish low heap as the common reset cause.
- Verified access19 rows form boot796 source sequences1–19 across five sessions; two independently signed sensor summaries and eight mobile incidents (two failures/six TARGET_NOT_READY skips) are available. Access/Target incident cursors exhausted in one page; event/mobile-incident/sensor truncation=false. Historical mobile-report and incident-internal health caps are truncated; independent health pagination covers this window. New44401 reports152–161 contain four new mobile sessions and22 wake callbacks with screen_interactive=false. Exact session matching now demonstrates screen-off collection and authentication, not physical-door movement or owner arrival.
- Sessionc5fcaa2a-81b3-412d-be6b-d52a95ba7631 matches mobile08:17:10.987–13.359 success and all eight Target flow events: GATT→ARM260ms, ARM→sensor7589ms, sensor→relay2ms, pulse1016ms; signed summary59 samples/48 timeouts/11 valid/3 in range. Sessionefd4351e-8bff-45a9-87ed-a3c00a2f50ae matches mobile08:19:16.783–22.318 authentication success, followed by ARM_TIMEOUT after59999ms:453 samples/448 timeouts/5 valid/0 in range, valid1405–2658mm versus800mm threshold, blocked at both start/end, terminalFAULT. This localizes the incomplete flow after authentication; aggregate measurements cannot establish physical presence, electrical cause, or rearm block as the sole cause. Later signed status confirms terminal reason; advisory accepted receipts9→21 accounts for the additional canonical/sensor records, while backpressure2 is not evidence of loss in this complete chain.
- Three other Target sessions terminate prechallenge with OTA_BUSY. Mobile additionally records TARGET_BUSY and GATT_DISCONNECTED133 (three attempts each), without Target session IDs for one-to-one attribution. Source audit confirms GattProtocol.cpp1339–1352 emits generic kBusy when beginAuth fails,1694–1695 maps it to kOtaBusy; TargetAccessFsm.cpp97–99 rejects any non-IDLE session, so OTA_BUSY alone cannot demonstrate an OTA attempt. ARM admission main.cpp620–632/FSM111–127 omits passageRearm, although radio readiness main855–856 and sensor-to-relay gate833 enforce it. Proposed follow-up: distinct active-session versus OTA busy reasons, explicit blocked-sensor ARM outcome/readiness, and bounded clearance/timeout chronology. Preserve authenticated proof, OTA priority and no-echo denial; do not clear the interlock merely on timeout. No such fix is implemented by this read-only check.
- Latest report161 captured08:28:24.358 and received08:28:24.654 KST; its prior upload ACCEPTED is08:25:00.459, pending0/dropped0 at capture. The38m06.161s receipt gap to the fixed window end is unobserved mobile time, not a current healthy state or proven OS/upload failure. Native scheduling acceptance and stored background_restricted=false do not cover the later gap. Only this observation appended; preserved existing edits, no device/control/OTA/reboot, code/configuration/deployment changes, commit/push or owner-log request. Whitespace validation follows.

## [2026-09-09] test | Quiet checkpoint through 10:08 KST with unchanged evidence gaps

- Fixed UTC2026-09-09T00:06:30.815Z–01:08:01.646Z: health119 rows/IDs1088–1206, two pages exhausted, latest receipt10:07:31.846 KST, max adjacent gap37.175s. All verified cores remain boot799/ffbb9699a1ddd337a78a986408d70d09, IDLE/relay commanded OFF; advisory480 uptime499→4149s. Readiness exacta105539/all12 checks true/Target fresh. No new boot, firmware transition or access counters. Existing reconnect/headroom pattern continues: MQTT3→16/attempts18/failures0/Wi-Fi outages0; minimum-ever heap33604→4004B, latest free63868B/largest26612B. Same-boot BROWNOUT and sensor FAULT90/UNKNOWN23/CLEAR6 are not new failure causes or physical results.
- Access/Target incidents/mobile incidents/sensor summaries each0; access and incident cursors exhausted in one page. Event/mobile-incident/sensor truncation=false; historical mobile-report and incident-internal health caps remain truncated, with health covered independently. Latest mobile161 unchanged; no detail refetched. Its last receipt08:28:24.654 is1h39m36.992s before the window end, an extended unobserved interval without new OS/upload-cause evidence, not present phone health or proof of no arrival. Quiet completion without repeating previous alerts. Only this observation appended; existing edits preserved, no device/code/configuration/deployment change, commit/push or owner-log request. Whitespace validation follows.

## [2026-09-09] test | Observe rapid resets and sensor timeouts without rearm blocking

- Fixed UTC2026-09-09T01:08:01.646Z–02:09:02.599Z: health124 rows/IDs1207–1330, two pages exhausted, latest receipt11:08:58.507 KST, max adjacent gap40.881s; verified IDLE117/ARMED5/RELAY_HOLD1/COOLDOWN1. Readiness exacta105539/all12 checks true/Target fresh. Seven new verified boots800–806 first received10:14:00.479/10:21:25.261/10:38:26.485/10:40:51.017/10:44:35.536/10:44:50.838/10:47:43.650 KST, all advisory480. Six have matching non-retained UNSIGNED BROWNOUT9/planned_restart none; boot804 has only one early status with unsigned BROWNOUT and no boot_observation. Boot805 reports previous uptime13497ms, consistent with a short preceding boot but not a measured power trace. Boot803/806 minimum-ever heap1040/1352B is lower headroom, not established reset causation. Latest boot806 uptime1281s/IDLE/free63444B/largest26612B.
- Access22 rows/source sequences1–22 across five boot801 sessions plus three independently signed sensor summaries are preserved, consistent in count with advisory accepted receipts25/backpressure3, not evidence of queue loss. Two ARM_TIMEOUT sessions80ea2358…/ec001ac6… wait60025/60059ms: samples454/457, timeouts437/417, valid17/40, in-range0/0, valid ranges1027–2891/944–2786mm versus800mm threshold. Both signed summaries show blocked start/endfalse and blocked_samples0, terminalFAULT: the earlier rearm interlock cannot explain these two outcomes. Complete session5d402eec… reports ARM→sensor20907ms, sensor→relay8ms, pulse1028ms;160 samples/149 timeouts/11 valid/5 in range. Two other sessions report the previously identified coarse OTA_BUSY, not proof of OTA installation. Arrival, actual sensor occupancy and physical door movement remain unobserved.
- Access/Target incidents each one page with cursors exhausted; event/mobile-incident/sensor truncation=false. Historical mobile-report and incident-internal health caps are truncated, with full health covered independently. Mobile incidents/new reports0; latest161 remains08:28:24.654 receipt,2h40m37.945s before window end, without a new upload/OS-cause observation. No new session matches. Bounded comparison prompted source verification: GattServer.cpp223–228 and GattProtocol.cpp512–566 derive credential_ref from door+normalized session+credential, so unequal references across sessions are expected and cannot identify another phone/person. Do not repeat cross-session equality as an identity test.
- New evidence prioritizes sensor-validity/timeout chronology and independent supply/reset observation alongside the already proposed truthful ARM/busy-state changes; changing the rearm gate alone cannot resolve these unblocked timeouts. Preserve no-echo denial and OTA/rollback. Only this observation appended; no device/control/OTA/reboot, code/configuration/deployment change, commit/push or owner-log request. Existing edits preserved; whitespace validation follows.

## [2026-09-09] test | Quiet checkpoint through 12:10 KST without another boot transition

- Fixed UTC2026-09-09T02:09:02.599Z–03:10:33.488Z: health120 rows/IDs1331–1450, two pages exhausted, latest receipt12:10:25.037 KST, max adjacent gap34.603s. Every authenticated core remains boot806/32e614ec2b3dd2fde80794a4c09beb3f, IDLE/relay commanded OFF. Readiness exacta105539/all12 checks true/Target fresh. Advisory480 uptime1312→4968s; latest free63476B/minimum-ever1352B/largest26612B, MQTT connects5/attempts6/failures0/Wi-Fi outages0. Same-boot BROWNOUT and refreshed boot-message receipt are not another reset. Latest advisory advertising active/expected true, no GATT/proof/sensor/relay counts, sensor FAULT/unblocked; the existing IDLE raw-reader counter gap remains, so zero samples does not prove an unpolled sensor.
- Access/Target incidents/mobile incidents/sensor summaries each0; access and incident histories each one page with cursors exhausted. Event/mobile-incident/sensor truncation=false; historical mobile-report and incident-internal health caps remain truncated, with health covered independently. Latest report161 remains captured08:28:24.358/received08:28:24.654 KST,3h42m08.834s before window end. No new report detail fetched, mobile session match, or upload/OS-cause evidence. This is unobserved mobile time, not current phone health or proof of no arrival. No newly actionable failure/install/physical result; quiet completion without repeating previous alerts. Only this observation appended; preserved existing edits, no device/control/code/configuration/deployment change, commit/push or owner-log request. Whitespace validation follows.

## [2026-09-09] test | Quiet checkpoint through 13:12 KST with an unattributed prechallenge connection

- Fixed UTC2026-09-09T03:10:33.488Z–04:12:04.384Z: health118 rows/IDs1451–1568, two pages exhausted (two additional bounded pages rechecked GATT transitions), latest receipt13:11:51.737 KST/max gap41.501s. All verified cores remain boot806/IDLE/relay commanded OFF; readiness exacta105539/all12 checks true/Target fresh. Advisory480 uptime5002→8654s, MQTT connects5→20/attempts24/failures0/Wi-Fi outages0; latest free60176B/minimum-ever1352B/largest26612B. No new reset/version transition. Unsigned GATT stage CONNECTION_ACCEPTED→DISCONNECTED spans6663ms, first sampled12:10:59.546 then12:11:29.550; active connections1→0 and advertising resumes. Challenge/proof/terminal counts0/session null: a bare prechallenge connection, not a proven authentication failure or identified phone/person. Sensor UNKNOWN23/FAULT95 and the IDLE counter limitation remain unchanged concerns.
- Access/Target incidents/mobile incidents/sensor summaries each0; both event/incident cursors exhausted in one page. Event/mobile-incident/sensor truncation=false; historical mobile-report and incident-health caps true, with health covered independently. Latest report161 captured08:28:24.358/received08:28:24.654 KST remains4h43m39.730s before window end; no new details fetched or fresh mobile match/upload-cause evidence. No newly actionable failure/install/physical result; quiet completion without repeated stale-state alerts. Only this observation appended; existing edits preserved, no device/code/configuration/deployment changes, commit/push or owner-log request. Whitespace validation follows.

## [2026-09-09] test | Quiet checkpoint through 14:14 KST with unchanged incident evidence

- Fixed UTC2026-09-09T04:12:04.384Z–05:14:05.434Z: health121 rows/IDs1569–1689, two pages exhausted, latest receipt14:14:04.326 KST/max gap36.970s. All verified cores remain boot806/IDLE/relay commanded OFF; readiness exacta105539/all12 checks true/Target fresh. Advisory480 uptime8685→12388s, latest free63508B/minimum-ever1352B/largest26612B, MQTT connects20→34/attempts39/failures0/Wi-Fi outages0. No new boot/GATT/proof/relay activity. Sensor UNKNOWN21/FAULT95/OCCUPIED2/CLEAR3 are unsigned raw-state observations, not identified arrival or physical-entry evidence; the IDLE counter limitation remains.
- Access/Target incidents/mobile incidents/sensor summaries each0; access and incident cursors exhausted in one page each. Event/mobile-incident/sensor truncation=false; historical mobile-report and incident-health caps true, with full health covered independently. Latest report161 captured08:28:24.358/received08:28:24.654 KST remains5h45m40.780s before window end; no new detail fetched or new upload-gap cause observed. Quiet completion without repeating unchanged alerts. Only this observation appended; existing edits preserved, no device/code/configuration/deployment changes, commit/push or owner-log request. Whitespace validation follows.

## [2026-09-09] test | Evening manual-entry incident and BLE advertising recovery evidence

- Direct read-only API coverage UTC05:14:05.434–12:58:52: 914 verified health rows1690–2603, ten pages exhausted, latest21:58:40.985 KST/max adjacent receipt gap35.645s; readiness exacta105539/all12 checks true. Two new boots807/808 first received15:47:41.261/15:53:50.869; their unsigned BROWNOUT advisories do not measure voltage or establish reset causation. Boot808 persists through the evening incident. Local DNS failed; a Windows-resolved address used only inside the HTTPS client restored reads while preserving hostname/SNI/certificate/redirect/token checks, without system or credential changes.
- User reports failed automatic entry followed by manual opening. Boot808 signed health confirms manual relay ON21:52:20.866, OFF/completed21:52:21.796 and IDLE21:52:31.955, terminal session b7370c45-b805-43b6-953d-729cf0b84d82/sequence2/phase0x18. Manual relay actuation itself sets the sensor rearm interlock: blocked=false at21:40 and true afterward must not be retroactively blamed for the pre-manual failure. Physical entry is the user's report, not independently proved by relay telemetry. New mobile reports162–166 arrive21:51–21:57 with app44401; latest capture21:57:42.815/receipt21:57:43.267 is fresh but its last BLE packet08:20:09.457, wake08:20:19.465 and session update08:20:02.903 are old. Scan registration accepted21:51:38.897/21:52:15.118 without a new packet/session. The ~13-hour mobile evidence gap and its cause remain unresolved; old GATT failure is not a new evening authentication result.
- A separate actual audit identity collision is confirmed by source and stored evidence. Boot808 canonical GATT pre-proof failure events occupy sequences1–8 (sessions abcb5d14… and fe11accc…), while later signed manual terminal health reuses sequences1/2 (847902a3… and b7370c45…). GattProtocol.cpp1657 allocates GATT sequences independently; GattServer.cpp358–365 updates MqttManager's highwater only after a proof-verified phase tracker terminal, excluding pre-proof failures. MqttManager.cpp2450 therefore allocates conflicting manual positions; Backend migration011's unique Target/boot/sequence and main.py1123–1203 reject nonidentical duplicates. Manual canonical events are absent from the fully queried boot window. A shared allocator and regression covering pre-proof failures followed by manual actions are needed, not a weakened Backend uniqueness check. Exact current NVS queue head is unobserved: mqtt_event_outbox_depth is only the RAM queue, so zero depth/ACK count8 do not establish durable queue emptiness.
- Added HA photo explicitly shows advertising OFF18:02:39/ON18:02:40/OFF18:02:41/ON18:02:42, not an unavailable label. Bounded incident read18:01:30–18:04:30 has health coverage complete: between receipts18:02:15.990 and18:02:48.311, unsigned advertising restarts/successes5→8, watchdog recoveries2→5, failures0, accepted connections5→6, disconnects2→3, challenges1→2 and proofs0. Verified session fe11accc… ends GATT_DISCONNECTED before proof, source span1162ms, received18:02:48.443–49.331; no matching mobile report or owner attribution. These independent receipt and HA transition times are not exact physical-event timestamps. GattServer.cpp805–815's watchdog requires expected advertising/no controller-counted connection/inactive advertising; ordinary disconnect restarts do not increment watchdog recoveries. Thus the three watchdog recoveries should not be dismissed as one normal disconnect restart. A callback/controller-count timing race is a source-supported candidate, not an established field cause.
- Additional bounded incident read21:48–22:12 returns50 health samples, health/event/sensor truncation=false, latest22:11:59.102. Same boot808, advertising active/expected=true, connections0, restart8/watchdog5 and no new GATT challenge/proof across the manual action. HA home_assistant_bridge.py540–565 displays unsigned Target internal advertiser state with30s expiry; neither this nor GattServer.cpp761–770's controller query proves RF reception at the phone. Prioritize advertising stop/restart reasons and connection lifecycle, mobile no-packet freshness/recovery and durable audit visibility; do not prescribe a sensor threshold change or claim the sequence fix would resolve BLE non-detection. Analysis only: preserved existing edits, no device/control/OTA/reboot, code/configuration/deployment changes, commit/push or owner-log request. Whitespace validation follows.

## [2026-09-09] compile | Debuggability priorities and read-only MQTT access proposal

- Extended hands_free_reliability_recovery_2026_09_08.md section10 with evidence-driven priorities: shared Target sequence allocation, durable queue/DB rejection observability, BLE GAP/restart lifecycle, mobile capture-versus-upload freshness, IDLE sensor aggregates, reconnect reasons and privacy-safe identity/time correlation. Existing deployed collection is distinguished from proposed additions; no runtime implementation or renewed physical-result claim.
- Audited broker template, Backend subscription/API paths and local tool availability. Paho is present; MQTT environment inputs are absent; provisioning file declaration presence was checked without printing secret values. Historical MQTT success is not current dedicated observer authorization. Existing HTTPS read token does not grant MQTT access. Proposed separate diagnostics credential/CA/reachable TLS endpoint, exact-topic subscriptions, unique client ID, NAS-side history and a bounded read-only client; new environment names are explicitly unimplemented proposals.
- Found an onboarding pitfall: current global Mosquitto pattern write rules apply to new principals too, so adding user-scoped reads alone is not complete read-only authorization. Verified semantics against official Mosquitto ACL documentation; require deployed-version-aware isolation/deny and isolated broker regression without production effect-topic publish. No broker login, account/configuration/firewall change, command, deployment, commit or push performed. Existing unrelated edits preserved; documentation links and whitespace checked after editing.

## [2026-09-09] compile | Explain conditional Mosquitto diagnostic-account onboarding

- Added owner instructions for existing password_file/acl_file brokers: verify actual container/mounts/active settings and back up privately; interactive mosquitto_passwd without destructive -c or exposed -b; preserve existing principals and append exact diagnostic reads. Template-specific own-namespace deny neutralizes inherited %u writes without deny-all blocking required reads. Official Mosquitto manuals checked for password, deny precedence/global patterns and SIGHUP reload semantics.
- Container/path examples are explicitly conditional, not asserted live configuration. No account/password generation, broker connection/reload, operational ACL change, device command or publish performed. Existing edits preserved; whitespace validation follows.

## [2026-09-09] compile | Correct MQTT onboarding assumptions from owner configuration

- Owner supplied actual listener1883/8883 configuration with allow_anonymous=true and server TLS files, without password_file/acl_file. Distinguished it from repository template; if no additional configuration/plugin exists, anonymous subscriptions need no new account and TLS does not enforce client authorization. Explained observer read-only behavior versus broker-enforced read-only privilege and phased migration preserving all existing Target/HA/Backend principals. No remote verification, broker connection/account/configuration/reload or device action performed; appended clarification and checked whitespace.

## [2026-09-09] test | Direct MQTT observation confirms repeated conflicting manual audit record

- Owner-authorized45s read-only TLS observation22:27:33.604–22:28:18.752 KST: current DNS and CA/hostname verification pass at tworimpa.synology.me:4883, anonymous CONNECT0 and11 exact-topic SUBACKs QoS1. Unique client ID/clean session, no username/key reuse, publish/will/control/OTA/reboot or broker/account/configuration change.42 non-retained Target statuses/max gap1.292s;43 verified bridge statuses including1 retained. Connection closed at observation end.
- Canonical event34e02f7c-7bb5-46fa-b03f-cebe8712fd8e repeats21 times non-retained/~2s, boot808/sequence1/session847902a3-7081-4513-9920-6401a4f3c683/ACCESS_SIGNED_MANUAL_COMPLETED. Full boot808 API query still8 verified GATT rows/sequence1 already occupied/cursor exhausted, manual absent. No command receipt/command-ack/bridge access-event message observed. Firmware durable-head retry source matches repeated old record; exact Backend reject branch remains unobserved. Local MAC check skipped because required local provisioning combination unavailable; raw event is not newly claimed MAC-verified.
- Authenticated health confirms current boot809;33-row22:12–22:29:04.734 query exhausted, first boot809 receipt22:19:07.076/uptime7s. MQTT sample480/IDLE/advertisingtrue/connections0/GATT0, RAM outbox0/accepted audit receipts0 despite earlier-boot replay. Current readiness12 checks true/exacta105539; report177 freshly captured22:27:42.838 and received22:27:43.318 but last BLE packet/session remain08:20. Neither ready nor report upload proves audit progress or BLE reception.
- Updated recovery proposal section11 and navigation: P0 shared sequence plus preserved conflict quarantine/recovery, audit-health versus status-health, BLE/mobile stage freshness, reset/sensor diagnostics, then NAS capture and staged broker authority. No firmware/backend/mobile implementation, deployment, commit/push or physical trial. Existing unrelated changes preserved; document links and whitespace validation follow.

## [2026-09-09] fix | Shared access sequence and durable audit conflict recovery

- Implemented one boot-scoped AccessEventSequence shared by GATT and signed manual completion under the adapter recursive task mutex. Pre-proof failures now advance the same allocator; MQTT initialization/session resets do not reset it, and uint64 exhaustion cannot wrap. Existing event IDs/MACs/368-byte NVS layout, asynchronous evidence drain, relay and OTA contracts remain unchanged.
- Added schema017 immutable access_event_conflicts and MAC-verified canonical collision custody. Exact normalized payload/digest replay is idempotent; commit success alone permits the existing exact-event diagnostic receipt. Failed/uncertain storage does not ACK. Quarantine does not mutate canonical history or drive normal HA/access-success callbacks. Down migration preserves acknowledged evidence.
- Added read-token GET /api/v1/diagnostics/audit-conflicts and PC --audit-conflicts with bounded receipt-time/identity filters, cursor, shared authorization/rate limit/no-store and no raw envelope/MAC/key exposure. Added periodic unsigned audit durable/total depth, current-boot head observation wait, attempt count, head boot count and 15-second pending advisory to Backend health history. Added core-ownership/mutex guard to BLE watchdog restart for SDK onConnect/count-update gap; this is not field RF-causality proof.
- Verification: Backend273 tests passed (4 environment skips); common392 tests passed (1 environment skip); separately enabled isolated MariaDB conflict suite6 tests passed. Real DB test preserves two conflicting MAC-valid manual events, verifies3 replays each produce2 quarantine rows only, accepts a following normal event, and leaves3 canonical rows/1 HA outbox row. Immutable UPDATE/DELETE rejection, repeated/up/down migration, uncertain commit refusal, forged/retained refusal and exact receipt routing verified. C++ tests cover pre-proof/manual/GATT sequence interleave, overflow and old-boot head wait. Personal-production and BLE-disabled ESP32-C6 builds pass; OTA contract PASS. Live read-only status size check3119B/108 fields fits existing5632B transport headroom with added fields; no live publish/control used.
- Updated recovery section12, diagnostic API documentation and index; relative links and whitespace pass. DB Dockerfile and Backend input inventory updated for017; trusted authorization policy, NAS wrapper, broker configuration and secrets untouched. Deployment requires the existing protected-input admission and Backend/schema-first ordering. No commit/push, production deployment, OTA, reboot, door action or mobile change performed. Pre-existing unrelated wiki changes preserved. local observation recorded.

## [2026-09-09] compile | Begin owner-authorized audit recovery deployment
...
## [2026-09-09] test | Quiet read-only follow-up after audit deployment

- Fixed UTC2026-09-09T05:14:05.434Z–14:25:01.834340Z backfill: health1000 rows/10-page cap reached, IDs1772–2771 covering05:56:33.300–14:24:43.118. Earlier05:14:05.434–05:56:33.300 remains unconfirmed; continue before_id1772 with the same fixed window on a later run. Access16 rows/one page exhausted, latest09:02:49.331; audit conflicts2 rows/one page exhausted, both the already reported boot808 manual conflicts recovered during deployment. No new access event after the previously analyzed evening failures. API receipt time is not physical occurrence time.
- Readiness exact69f4703/all12 checks true/Target fresh. Latest verified state remains boot809/b7231d42ed9986447ec5030a51def2a4/IDLE/relay OFF; advisory480/uptime3943s/audit receipts2/rejected0/RAM and legacy queues0/advertising active/no connections. Backfilled boot807/808/809 and two GATT_DISCONNECTED sessions plus one FLOW_REPORTED_COMPLETE are already known from manual analysis, not new alerts. Published485 installation remains unobserved after the previously accepted single OTA request; no new reboot or worsening is established.
- Incident event/sensor/mobile-incident truncation false; incident health and mobile-report truncation true. Independent health backfill has the explicit older gap above. Latest opted-in report192 captured23:22:42.879/received23:22:43.312 KST/app44401 has upload ACCEPTED/pending0/dropped0, but last BLE packet08:20:09.457 and failed session08:20:02.903 remain historical. Fresh upload does not prove a fresh scan/arrival. No notification repeats the already reported pending installation/reboot decision. No control/OTA/reboot/config/code change or commit/push; preserved unrelated documentation changes.

## [2026-09-09] test | Investigate accepted OTA without Target installation

- Rechecked current OTA call path and480→485 source diff: OTA/main/config unchanged. Command ACK means request queued, not execution or installation. Failures remain internal FAILED/lastError plus Serial-only logs; current remote health has no OTA stage/error/progress. Flash begin failure is also folded into artifact HTTP/size. Documented this confirmed observability/classification defect without inventing the device failure stage.
- Backend23:17–23:29:38 query25 rows exhausted, all480/boot809/IDLE/MQTT connections2; stack watermark5956→3864 around request is not a crash or execution-stage proof. Strict HTTPS PC same-TCP manifest1118B/artifact1918148B transfer23:29:39 succeeds with exact digest; does not establish ESP32 TLS/heap success.
- Passive MQTT23:33:23.882–23:34:07.629 returns41 non-retained statuses, same480/boot809/IDLE; no install or reset. Brief heap54484B then63184B and largest19956B cannot identify retry execution or allocation failure. Current48KiB/32KiB boot-health criteria are post-reboot, not an OTA preflight gate. Added recovery section14 with persistent stage/error diagnostics and measured TLS resource handoff proposals. No duplicate OTA, reboot, AP switch, door action, runtime edit, deployment or commit/push; unrelated edits preserved. Whitespace checked.

## [2026-09-09] fix | Implement persistent OTA diagnostics and scoped TLS resource recovery

- Owner requested reinforcement, firmware publication and a new update attempt. Added independent bounded checksum NVS OTA record with attempt/boot/time, current/failed stage, HTTP/TLS/flash/rejection codes, artifact progress and heap handoff observations. Stage writes plus256KiB progress checkpoints survive ordinary reboot; corrupt/invalid records are refused. Advisory storage is separate from anti-rollback authority. Existing history API projects and revalidates the closed ota object without credentials or error text.
- Remote OTA now acquires a safe main-task-only transport lease after connect/status workers relinquish ownership. It releases established MQTT TLS memory, prevents reconnect/status-worker overlap, discards unadopted worker results and resumes reconnect after OTA client destruction on normal/error exits. No automatic reboot-on-error, signature relaxation, active-slot overwrite or changed rollback thresholds. Local recovery keeps its existing transport path with diagnostic checkpoints. Expanded prepared status wire buffer5632→6656 within existing8192 MQTT capacity so sensor and OTA evidence coexist.
- Initial verification: common395 tests passed (1 environment skip), Backend274 passed (4 environment skips), personal and BLE-disabled ESP32-C6 builds pass, OTA contract PASS. Added real host record-corruption checks and scoped fake-transport cleanup across8 failure exits/normal/busy plus source ownership gates; reran affected tests after final cleanup. Final build/check and exact-main publication/install observation follow. No mobile/broker configuration change or physical door action. Unrelated continuous-presence documentation preserved unstaged.
- Final review adds current-slot running_image_valid from actual ESP OTA state/mark-valid success, independent of the last check stage, so the next periodic check cannot hide boot-health completion. This remains unsigned advisory with explicit integrity labeling. Both final profile builds passed after transport-buffer adjustment; affected tests/build and CI validate the final slot-state addition.

## [2026-09-09] fix | Align prepared and worker telemetry capacity after OTA reinforcement

- PR399 merged106a2d2fa7f635c05d21e77d6f3c2a4ac9bb616a. Target487 publication34366134649 succeeded23:54:21–23:55:04 KST; Backend34366134858 NAS stage23:53:39–23:55:15 succeeded and independent ready confirms exact SHA/all checks. Exact encrypted artifact1922900B/SHA2565a57c0adb7e7a34e41ca2367ce55540a1e716671ec5982f53f5d325e67985564 independently verified.
- Matched fresh non-retained raw/verified boot809/IDLE/relay OFF before one new owner-authorized OTA intent23:56:15.052. Signed session16b424ad5b4f7d3d38ff1ef71c087510 received result0/target_accepted. Subsequent48 raw statuses through50s remain480/boot809, not install proof. Owner explicitly approved one safe remote reboot if further observation remains unchanged; not yet performed.
- Final cross-path inspection found prepared6656B versus worker6144B limit could suppress unusually large ARMED telemetry. It cannot explain the currently installed480 OTA failure. Unify producer/worker on one6656B constant under the8192B packet budget, add actual maximum6655B acceptance/6656B rejection worker tests plus source consistency assertion. Publish a corrected exact-main candidate before the approved single reboot. No relay command or changed safety thresholds; unrelated documentation preserved.

## [2026-09-10] fix | Update complete telemetry budget test for shared capacity

- Candidate488 main0783bb7 build34367286253 stopped before publication because the pre-existing sensor budget test parsed a literal producer array length and a private worker constant. Runtime maximum-boundary worker test and local personal build passed, but the old test reader did not understand the new shared declaration. Correct the test to read the single header constant, require both consumers to use it, and include the entire OTA diagnostic object and signed-code worst cases alongside the full sensor summary; do not bypass the budget assertions.
- Published487 and installed480 remain unchanged; no reboot or duplicate OTA sent. Local Target address was observed through MQTT, but a bounded unauthenticated HTTP connection timed out; local recovery also requires AP-active state, so no credential submission/AP activation/upload attempted. Owner-approved single safe reboot remains pending corrected publication. local observation recorded.

## [2026-09-10] test | Published489 and single approved reboot did not install the new image

- PR401 final main1e8b8e1 / run34367682991 passed and published489 at00:11:24–00:12:07 KST. Independent strict HTTPS encrypted1922900B/SHA25656f9dda9284d86689c9884d03f5ea3189189996763708b4774b9a22d66ca6a6b match. Backend remains deployed106a2d2 because the final changes are Target/test-only.
- Used the one explicitly approved safe reboot00:13:51.146 after fresh raw/verified IDLE/relay OFF. Signed sessiona72ac2714c19fa7c037d5e20393723bc accepted; boot810/98bf019432cbfb6abe8e1d785a8463c8 reports480/SOFTWARE/signed_mqtt_reboot. No additional OTA/reboot/AP/door action.
- Automatic OTA scheduled near uptime60s was followed by status silence. Backend row2867 at00:14:30.291/uptime37 is followed by2868 at00:20:00.652/uptime368, same boot/version. Min heap46184→2096B, largest50164→33780B, MQTT connections1→2, Wi-Fi outages0. Time/heap correlations do not establish timeout/allocation failure; installed480 has no new OTA checkpoint. Fresh raw/verified status recovered through00:21:32; ready exact106a2d2/all12 checks/Target fresh. Installation and health remain unconfirmed, not a completed upgrade or observed rollback.
- Updated recovery section15, status, hardware evidence and index; preserved unrelated continuous-presence edits. Further bootstrap requires reachable authenticated local recovery or USB; no user log-copy request. Documentation-only publication does not request another firmware build. local observation recorded.

## [2026-09-10] test | Owner explicitly authorizes one additional OTA attempt

- Revalidated published489 manifest/encrypted1922900B and SHA25656f9dda9284d86689c9884d03f5ea3189189996763708b4774b9a22d66ca6a6b over strict HTTPS. No rebuild or publication change.
- Before this new command, fresh raw/verified status already showed boot811/e13f39998a061147494e7efd08f5ec90, old480, uptime12, IDLE/relay OFF and reset BROWNOUT. Backend row2880 at00:26:08.749 still showed boot810; row2881 at00:26:31.532 reports the new boot. This reset predates the OTA request; no reboot command was sent this turn.
- Sent exactly one non-retained HA bridge trigger_ota intent00:26:33.265 KST after matching fresh safe-state evidence. Signed session72055eb1b59b9be314ade0bbfae3eaab received broker acceptance, Target result0 at00:26:33.737 and target_accepted. Passive install/health observation continues; acceptance alone is not installation. Unrelated continuous-presence edits preserved.

## [2026-09-10] test | Additional OTA attempt returns same480 after five-minute status gap

- Raw status returned00:31:36.405; Backend row2882 received00:31:36.547, same boot811/480/uptime315s, IDLE/relay OFF. Row2883 at00:32:07.143 confirms uptime346s/MQTT connections2/Wi-Fi outages0. No489 install, new image boot or rollback observed. Min heap1080B/largest29172B after attempt versus preflight min59880B/largest58356B is pressure evidence, not an allocation/download failure code. Approximately303s since command is compatible with the5m download bound but not proof of that internal error.
- Matching retained boot811 advisory separately reports BROWNOUT/planned none/previous uptime747926ms/previous action mqtt_connect_worker_adopted/previous relay OFF. Coredump exists but does not match this reset. Reset predates this turn's OTA command; do not attribute it to the new request or claim a specific power component without measurement.
- Updated recovery section16 and hardware evidence; no code/build/publication change, extra command, reboot/AP/door action or log-copy request. Existing unrelated edits preserved. local observation recorded.
- Observation ended00:33:33.318 KST after420s:112 non-retained raw statuses, maximum gap303.42s, final status age0.6s, unchanged480/boot811. Fresh authenticated bridge IDLE/relay OFF also observed through00:33:18. No install or further reset observed during this attempt. Whitespace check passed; evidence changes remain local.

## [2026-09-10] test | Quiet read-only heartbeat after the reported OTA retry

- Fixed receipt window UTC2026-09-09T14:25:01.834340Z–15:35:41.657692Z: health118 rows/2 pages (IDs2772–2889), access0/1 page and audit-conflicts0/1 page, all cursors exhausted. Resumed prior health gap with before_id1772:82 rows/1 page (IDs1690–1771,05:14:35.064–05:56:02.662), cursor exhausted under the original05:14:05.434–14:25:01.834340 window. Previously capped pagination gap is now covered; empty event sets are not physical-access proof.
- Ready remains exact106a2d2/all12 checks true/Target fresh. Latest health00:35:11.387 KST remains480/boot811/e13f39998a061147494e7efd08f5ec90/uptime530/IDLE/relay OFF, min heap1080/largest29172, BLE advisory active/no connections. Boot810 authorized reboot and boot811 BROWNOUT plus both OTA observation gaps are already reported, not new alerts; no later reset/install or worsening observed.
- Incident event/mobile-incident/sensor truncation false, health/mobile-report truncation true; independent health pagination above covers the fixed interval. No returned incident or explicit mobile runtime reason in that interval. Latest opted-in report210 captured00:32:42.934/received00:32:43.434 KST/app44401 confirms native heartbeat/upload ACCEPTED/pending0/dropped0. Last scan packet09/09 08:20:09.457 and failed session08:20:02.903 remain historical; fresh report receipt does not establish new approach/wake/physical entry. Mobile-report truncation remains an explicit coverage limit.
- Quiet because no new actionable finding beyond the immediately preceding user-facing result. Read-only API access and this append only; no device command, configuration/code change, deployment, commit or push. Existing user changes preserved; whitespace checked.

## [2026-09-10] fix | Remove reuse-only OTA transport dependency and resume bounded downloads

- Reviewed installed480/published489 source and the two approximately5m gaps separately. Confirmed latest code still required the manifest TLS socket to survive and aborted incomplete artifact transfers without same-attempt recovery. The old480 gaps/min heap1080B do not expose a precise OTA error; pre-request BROWNOUT remains a separate event. No claim that Range/reconnect alone proves the field root cause.
- Added production-used allocation-free C++ download pump: prefer existing verified socket, otherwise close old TLS and reconnect with the same CA/hostname-verified client under the existing MQTT lease. Resume immutable ciphertext with exact Content-Range/length validation, at most3 reconnects within5m; coalesce fragments into existing4KiB capacity while retaining buffered bytes and GCM/hash state. Reject ignored/overlapping/wrong-size/encoded responses; bound manifest Content-Length1..4096 and body encoding before getString. Flash errors are terminal. Signature, final ciphertext/plaintext digest/tag, inactive-slot selection, health/rollback, local recovery and pins unchanged. Resume is RAM/same-attempt only, not across power loss.
- Strict HTTPS PC Range0–63 probe of the existing published artifact returned206/exact range/64B. Host ASan/UBSan executes the same pump with1922900B at1/15/16/17/4096B fragmentation, partial-buffer disconnect/reconnect, exact output,470 writer calls, short/zero reads, initial failure, invalid range/encoding, retry exhaustion, idle, wrap-safe total deadline, slow open/read/write and flash failure. Corruption still requires the unchanged caller hash/tag check. This is not ESP TLS/crypto/flash or physical install evidence.
- Updated source-location-based tests to follow the extracted pump while retaining GATT/watchdog/deadline assertions. Final common396 run/395 pass/1 skip, both production profile builds PASS, OTA contract PASS. Updated contract/runbook/recovery/index/hardware evidence. No new device command, reboot, AP change, door operation, signed publication or commit/push. Preserved unrelated continuous-presence documentation; final review/whitespace checked.
- Final read-only Backend check finds a new reset during implementation: row2926 at00:54:07.009 KST remains480/boot811/uptime1665; row2927 at00:54:37.960 and2928 at00:55:08.643 show480/boot813/458ca50e65931bf549131cee5ff32588, uptime8→38/IDLE/relay OFF. Matching unsigned boot advisory reports BROWNOUT/code9/planned none/previous uptime1991ms/BOOTING/wifi_sta_profile_enabled/previous relay OFF. Boot812 was not remotely observed. No command from this turn caused an authorized restart; physical owner action and exact electrical component are unobserved. Keep this separate from the missing old480 OTA-stage diagnosis and do not present a software-only fix as sufficient for power-integrity failures.
