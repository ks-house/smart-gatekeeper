...
> Hardwareless RC for issue #17. The exact-main personal APK completed one foreground proof/result against the exact-main ESP32-C6; Samsung/OEM screen-off, process-death and repeated physical acceptance remain G0-HW gates.
...
## 8. Physical evidence and pending gates

On 2026-08-25 GitHub Actions run `32777471718` produced production-signed `1.0.0-gdb37bc2` (`versionCode=19001`) from exact main `db37bc2390efbf94bf1a9fca261834c3728606b5`. Its package, version, v2/v3 signature, pinned certificate and embedded source identity verified before `adb install -r`. The connected Samsung phone retained the original first-install time, package data, local consent and AndroidKeyStore credential; the app reported `local_keystore_authenticated` and native GATT ownership.

A foreground 1-Tap local request then connected to exact Target `2.1.262+main.gdb37bc2`. Android's Bluetooth trace recorded connection, service discovery, indication enablement for Hello/Challenge/Result, multi-frame characteristic writes, clean disconnect and WorkManager success. A fresh health projection reported `HEALTHY`, no failure reason, no Target denial reason and `4,599 ms` latency. In this state the durable worker can only clear both reasons after parsing authenticated Target reason `0`, so this closes the earlier callback-stack and `MALFORMED_PROOF` blockers for the observed foreground session. HA independently recorded Target `AUTH_PENDING` at 06:27:33, `ARMED` at 06:27:36 and return to `IDLE` at 06:28:35. The Target did not reset during the exchange.

This does not establish Samsung screen-off repetition, Activity/process-killed or reboot registration behavior, OEM battery-policy survival, latency percentile/SLO, physical sensor or relay operation, health-valid, power-loss or bootloader rollback. Personal enablement does not retire the legacy rollback path, and issue/Epic closure remains blocked by the remaining applicable G0-HW, RELAY-G0 through G2, OTA-G1 through G4, and issue #14/#23 device/operator evidence.
