...
`BleGattTransport` is the testable boundary between the state machine and Android Bluetooth APIs. When Fast RX/TX are present, production selects v2, enables only the Fast TX CCCD, receives the Target-generated challenge, writes the fresh signed proof to Fast RX and awaits the FSM-bound result on Fast TX. It never runs Client/Target Hello in that session and never falls back to v1 after a v2 error. If both fast characteristics are absent during the OTA N/N-1 transition window, it uses the isolated v1 hello/challenge/result flow. Neither path issues a simultaneous Challenge characteristic read. Protocol messages use the UUIDs, lengths, field ordering, unsigned integer encoding, SHA-256 inputs, ATT fragmentation, and reassembly rules in `security_protocol.md`.
