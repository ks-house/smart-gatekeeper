...
않습니다. `NAS_KNOWN_HOSTS`가 없으면 public canary는 bounded runtime `ssh-keyscan` 결과를 run-local 파일에 고정하고 `runtime-keyscan-unpinned`를 기록하며, `physical-test-connected`는
