...
The feature-ON `esp32c6_personal_production` PlatformIO build compiles the real BLE server, signed ACL verifier and adapter. Exact main `2.1.259+main.gbc9bb5d` was signed, published to NAS and installed by HA-triggered Target OTA. Corrected iBeacon bytes produced real Android exact-filter wake and GATT connection, exposing a repeatable `nimble_host` stack-protection reset during ACK-gated challenge output. ELF analysis found 2,736-byte output-drain and 3,216-byte canonical sink frames on the 5,120-byte host task. The stack-safe USB candidate `2.1.260-test.g163610d` was app-only flashed while preserving NVS/OTA data and fallback state; it restored Wi-Fi/MQTTS, ACL and GATT and did not reset during the same Target Hello/challenge connection. The subsequently hardened source also separates control from telemetry and defers advertising restart; a clean personal-production build used 1,780,268 bytes of a 7,340,032-byte slot (24.3%, 5,559,764 bytes headroom), with a 1,845,984-byte `firmware.bin` and 67,088/327,680 bytes RAM. These additional bytes are build evidence only until exact-main OTA installation.

The candidate observation proves only that this specific callback-stack reset did not recur through challenge delivery. The installed Android APK then exposed a separate simultaneous Challenge read/indication framing race and stopped at `MALFORMED_PROOF`; successful proof/result and FSM ARM are not yet claimed. The candidate is not exact-main signed OTA evidence. No repeated Samsung screen-off/process-killed run, relay/sensor trial, heap/soak test, power-loss/bootloader test, complete OTA-G1..G4, or RELAY-G0..G2 evidence is claimed. Issue #18 and Epic #13 remain open.
