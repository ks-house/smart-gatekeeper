...
> Updated: 2026-08-25
> Status: **personal production is compile/runtime ON and exact-main Android/Target completed one foreground proof/result without reset; repeated OEM, sensor/relay and rollback gates stay open**
...
The feature-ON `esp32c6_personal_production` build from exact main `db37bc2390efbf94bf1a9fca261834c3728606b5` was reproduced, signed and published by run `32777471683`. Its 1,845,920-byte plaintext uses 25.15% of a 7,340,032-byte OTA slot and leaves 5,494,112 bytes; the encrypted NAS artifact was atomically published and read back. HA's signed OTA control installed it and the Target rebooted as `2.1.262+main.gdb37bc2`, restored Wi-Fi/MQTTS and ACL, and started GATT enabled.

The matching production Android run `32777471718` was replacement-installed on the connected Samsung phone. A foreground manual session reached service discovery, enabled all three indication streams, exchanged the framed messages and ended with worker `HEALTHY`, no failure/Target denial and 4,599 ms latency. HA independently recorded Target `AUTH_PENDING` at 06:27:33, `ARMED` at 06:27:36 and return to `IDLE` at 06:28:35; no Target reset recurred. This closes the exact-main callback-stack and mixed Challenge-stream blockers for the observed session, but it is not sensor/relay evidence: no GPIO3 actuation or electrical measurement was captured. The current lifecycle callbacks also discard the bool returned by `handleAuthPending()`/`handleAuthSuccess()`, so fail-closed binding of a positive Result to actual FSM acceptance remains a source hardening Gate even though this observed session did reach ARMED. No repeated Samsung screen-off/process-killed run, heap/soak test, power-loss/bootloader test, complete OTA-G1..G4, or RELAY-G0..G2 evidence is claimed. Issue #18 and Epic #13 remain open.
