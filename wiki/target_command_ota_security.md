...
- A pending image enters a health window after reboot. It is marked valid only after all predicates—safe Target state, Wi-Fi plus MQTTS, at least 48 KiB aggregate free heap, and at least 32 KiB largest allocatable block—remain continuously true for 30 seconds with no sampling gap above five seconds. The gap stays well below the 45-second loop watchdog but no longer equals the one-second signed telemetry cadence. Any failed or stalled tick resets the healthy-since timer. A valid interval may complete at the exact 120-second deadline; any other state at equality rolls back, and a strictly exceeded deadline dominates a later healthy sample and invokes the ESP-IDF automatic rollback API. Controlled rollback preserves the last predicate that reset health (`safe`, `network`, `heap` or `sample_gap`) in boot diagnostics.
