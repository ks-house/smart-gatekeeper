...
> Last updated: 2026-08-30 (mobile remote physical success; fresh-install registration onboarding defect reproduced and corrected in source)
...
| Fresh family-member onboarding diagnosis | Connected fresh-install A24 (Android 16) had Bluetooth/location/notification permissions granted, exact app `1.0.0-gf403e10` / 33401, zero doors and a generated provisional AndroidKeyStore key, but Home rendered Backend unavailable without the registration action. Source showed status attached the provisional key and Backend returned 403 because that valid key was not enrolled | REPRODUCED before any registration/data mutation. Source now projects the supervised legacy registration state only when the valid credential ID is absent, while an existing credential ID with a different public key remains 403; focused 9 and full 149 Backend tests pass with two Docker-only skips. Protected review/deployment and connected UI readback remain pending |
