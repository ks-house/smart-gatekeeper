...
| DSM 24 multi-network MQTT diagnosis | Retained API logs show successful MQTTS provisioning validation followed by a synchronous subscriber `TimeoutError`, with no TLS/certificate/auth rejection. The production API alone is attached to external `edge` plus `internal: true` `data`; DSM Docker 24/Compose 2.20 predates `gw_priority`, while the legacy single bridge reconnects to the same host/4883 successfully | Root cause is consistent with the internal bridge winning the legacy Engine default route. Synology-only overlay correction keeps DB ports unpublished while allowing either API bridge to route outbound; protected CI and a new live attempt remain pending |
| DSM route-corrected source and deployment Gate | PR #239 head `2f0de8a` passed Hosted Trusted, OTA P0 and Backend checks and merged as feature main `aebad8ef`; final policy main `da39fe0` retains one 83-path baseline. Push run `33250299026` passed backend/security, evidence and immutable image publication and waits at protected production | PASS for source/CI/image publication only; owner-proved exact legacy stop, NAS apply, `mqtt=true`, deployed identity and backend-included access remain pending |
| Current WSL device inventory before retry | usbipd reports CH343 `1a86:55d3` attached and WSL exposes `/dev/ttyACM0`; a fresh nested WSL login has `dialout` and can open it, but 15-second filtered plus 6-second raw probes received zero bytes. Windows PnP and `adb devices -l` show no Android phone | PASS for Target USB transport presence only; no runtime serial identity. Mobile/ADB and connected E2E are unavailable until the phone is physically connected and authorized |
