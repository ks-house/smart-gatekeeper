...
> Last updated: 2026-08-30 (exact mobile OTA install verified; remote button reaches Backend but is denied before MQTT publication)
...
| Personal-scope source correction | Backend v3 now checks credential tenant and exact door grant against `ACL_PERSONAL_TENANT_ID`/`ACL_PERSONAL_DOOR_ID`, while retaining `COMMAND_*` only for the subsequent signed MQTTS publisher. Startup additionally rejects a personal scope bound to any Target other than `COMMAND_TARGET_ID`. The focused differing-scope/cross-Target regressions and all 149 backend tests passed with two Docker-only skips | PASS for source/unit regression only. Review, protected CI, NAS deployment, readiness and one owner-triggered connected button trial remain separate Gates |
