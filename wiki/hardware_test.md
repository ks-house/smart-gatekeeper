...
| Fresh family-member onboarding | Connected fresh-install A24 (Android 16) had Bluetooth/location/notification permissions granted, exact app `1.0.0-gf403e10` / 33401, zero doors and a generated provisional AndroidKeyStore key, but Home initially rendered Backend unavailable without the registration action. PR #295 corrected status and deployed exact main `bf435bf4...a9bf5` in run `33312971831`; canonical and independent readiness passed. The unchanged A24 then rendered the registration form, but the owner's first submission displayed `신청 접수에 실패했습니다` | PARTIAL: UI recovery passed, persistence did not. Source root cause is the request/API/storage mismatch: fresh IDs are `GK-*` UUIDs, the request allowed only `DEV-*`, and the legacy column is 17 characters. The proposed bounded-locator correction passes request, status and credential-enrollment regressions plus all 151 Backend tests with two Docker-only skips. Protected review, deployment, one owner retry, approval, signed ACL and daughter-device access remain pending |
