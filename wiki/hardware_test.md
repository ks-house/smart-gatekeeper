...
## 2026-09-04 Target connectivity self-recovery exact OTA rollout

| Test | Observed result | Verdict / boundary |
|---|---|---|
| Reviewed and deployed source | PRs #355, #356 and #357 passed hosted checks and merge-committed normally. Backend run `33883869053` deployed exact feature main `7774060ba580a64e925727dfbc17c7c045ed58e2`; strict-TLS `/live` and `/ready` returned HTTP 200 with all checks true | PASS for reviewed source, exact NAS process and dependency readiness |
| Signed Target publication | Run `33884109359` built, encrypted, signed, atomically published and HTTPS-read-back final main `57bfe105f005c9755e7765e6f1e28a2dd317b91c` as `2.1.445+main.g57bfe10` | PASS for exact signed publication; this row alone is not installation evidence |
| Single OTA request | One HA-bridge `trigger_ota` request was sent QoS 1 without retention. It received PUBACK and Backend `broker_accepted`; a Target ACK was not observed within the bounded 25-second wait, so no duplicate request was sent | PASS for single accepted request and duplicate suppression; missing ACK is recorded rather than reinterpreted |
| Exact installation identity | Strict-TLS MQTTS advanced Target `c0feffe6ebac` from boot 715 / `2.1.442+main.g9ef6b82` to boot 716 / exact `2.1.445+main.g57bfe10`, boot ID `dc9a95404b3372ce6d0595aa300b5e7c`. Retained boot diagnostics reported `SOFTWARE`, `ota_pending_verify` and prior safe IDLE/relay-OFF state | PASS for actual install, software reboot and restart breadcrumb |
| Post-update health and rollback | The same boot emitted 46 consecutive matching Target statuses plus Backend verified projections for 45.9 seconds, reaching uptime 112 without reset or version change. This exceeded the 30-second application health threshold | PASS for bounded healthy-window behavior and no observed rollback; no serial valid-mark line or power-loss rollback test was captured |
| Transport and fail-safe state | Wi-Fi stayed `CONNECTED`, HA bridge stayed `online`, and Target stayed IDLE with relay command OFF/pin high. MQTT connect failures, MQTT worker WDT failures, Wi-Fi outages/recovery escalations and evidence/RTC fallback failure flags were zero/false | PASS for the observed post-boot transport and safe-output interval; not a forced-outage soak |
| Physical boundary | No relay command, sensor passage, GPIO11/GPIO23 electrical measurement, actuator travel or door-leaf motion was performed | PENDING physical access and electrical acceptance; no door-open claim inferred |
