...
| DSM loopback-ingress wrapper install | Staged validation passed and owner readback shows exact feature-main wrapper SHA-256 `3e0fdd660316817493a5cc29e972fdcbfc90833621fb440a75bccc7875381bb5` installed as `root:root 0755`, 23,210 bytes. Exact wrapper status is `not-deployed` | PASS for installed script identity and metadata only. NAS-local `curl --resolve` ingress proof, maintenance stop, exact run approval, deployment/readiness and backend-included device E2E remain pending |
