...
| 관리자 force-open dual control | `SECURITY_OPERATOR` 요청자 + 별도 `SECURITY_APPROVER` | 두 세션의 verified mTLS·CSRF·재인증, 같은 tenant scope, reason, 서로 다른 idempotency key, safe scene | `POST /api/v1/admin/control/force-open`로 proposal 1회 생성 후 `POST /api/v1/admin/control/force-open/{approval_id}/approve` 1회 | proposal은 `approval_required`; 승인 후 `published`는 broker ACK 범위뿐이며 Target/relay effect 미확인은 `EFFECT_UNKNOWN`; `RECONCILIATION_REQUIRED`는 성공 아님 | #49 `backend/app/main.py`, `admin_security.py`, migrations 003–005 | #49 dual-control/audit host tests present; signed Target command + relay/operator event **PENDING** | proposal 만료 300초; 요청 HTTP 목표 15초, effect 관찰 목표 120초; 같은 idempotency proposal 재조회/재전송 최대 1회만, 승인·publication/effect 자동 retry 0회; 503/`RECONCILIATION_REQUIRED`/`EFFECT_UNKNOWN`이면 incident commander, backend control owner와 현장 안전 담당에게 approval ID·audit action·broker/Target event를 escalation |
