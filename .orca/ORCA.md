...
이 지침서는 Orca Multi-Agent 오케스트레이션 엔진을 통해 `smart-gatekeeper` 프로젝트를 체계적이고 효율적으로 수행하기 위한 **3대 에이전트 프로파일 (`gpt5.6-sol`, `terra`, `luna`)** 의 역할 분담, 설정 및 실행 워크플로우를 정의합니다.
...
| **`terra`** | **Terra** | ESP32-C6 펌웨어 (C++/PlatformIO/GATT/ToF/Relay) & 백엔드 (FastAPI/DB/MQTT/ACL Push) | Coding Model (Effort: `high`) | [terra.md](profiles/terra.md) |
| **`luna`** | **Luna** | Android Native (Kotlin/WorkManager/BLE Wake) & Flutter Thin UI & QA/E2E Fault Injection | Coding Model (Effort: `high`) | [luna.md](profiles/luna.md) |
...
graph TD
    ...
    Sol -->|"task-create & dispatch"| Terra["terra (Firmware & Backend)"]
    Sol -->|"task-create & dispatch"| Luna["luna (Android & Flutter & QA)"]
...
2. **워커 터미널 디스패치**:
   - `terra` 워커 실행: `.orca/scripts/launch_profiles.ps1 -Profile terra -TaskID <task_id>`
   - `luna` 워커 실행: `.orca/scripts/launch_profiles.ps1 -Profile luna -TaskID <task_id>`
3. **모니터링**: `orca orchestration check --wait`로 하트비트 및 `worker_done` 수신
4. **검증 & 병합**: 워커 결과물 리뷰, 자동 테스트 통과 확인 후 `gh pr merge --squash` 수행

### 3.2 워커 (`terra`, `luna`) 공통 작업 수칙
