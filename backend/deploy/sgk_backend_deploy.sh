...
validate_runtime() {
  ...
  # Optional capability; preserve existing runtime files until owner activation.
  RUNTIME[DIAGNOSTICS_READ_TOKEN_SHA256]="${RUNTIME[DIAGNOSTICS_READ_TOKEN_SHA256]-}"
  require_exact_keys RUNTIME \
    ...
    APK_ARTIFACTS_VOLUME MIGRATION_BACKUPS_VOLUME DIAGNOSTICS_READ_TOKEN_SHA256

  [[ -z "${RUNTIME[DIAGNOSTICS_READ_TOKEN_SHA256]}" || \
     "${RUNTIME[DIAGNOSTICS_READ_TOKEN_SHA256]}" =~ ^[0-9a-f]{64}$ ]] || \
    die "diagnostic read token digest must be empty or lowercase SHA-256"

  local key value
  for key in MQTT_HOST MQTT_PORT MQTT_USER DB_RUNTIME_USER COMMAND_TARGET_ID COMMAND_TENANT_ID COMMAND_DOOR_ID \
    COMMAND_SIGNING_KEY_ID ADMIN_TRUSTED_PROXY_IPS ACL_SIGNING_KEY_ID \
