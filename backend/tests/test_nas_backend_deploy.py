...
LATEST_SCHEMA_UP = (
    ROOT / "backend" / "db" / "migrations" / "018_mobile_evidence_window_up.sql"
)
...
class NasBackendDeployContractTest(unittest.TestCase):
    def test_schema_identity_is_manifest_derived_and_image_bound(self):
        ...
        self.assertEqual(("018", expected), create_release_bundle.schema_identity())
    ...
    def test_signed_bundle_has_exact_members_hashes_and_verifiable_descriptor(self):
        with tempfile.TemporaryDirectory() as raw_directory:
            ...
            self.assertIn("SCHEMA_VERSION=018\n", descriptor)
            self.assertIn("SCHEMA_SHA256=" + hashlib.sha256(LATEST_SCHEMA_UP.read_bytes()).hexdigest(), descriptor)
            self.assertIn(
        ...
        environment.update(
            {
                ...
                "BUILD_SHA": "c" * 40,
                "SCHEMA_VERSION": "018",
                "SCHEMA_SHA256": hashlib.sha256(LATEST_SCHEMA_UP.read_bytes()).hexdigest(),
