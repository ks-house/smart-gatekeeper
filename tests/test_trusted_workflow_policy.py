...
MERGED_MAIN_COMMIT = "df2ac4869f4ee15c567f4a5ce1e0a99fab08e269"
MERGED_MAIN_DIGEST_LINES = """\
.github/workflows/deploy.yml 26d9fc567b7465fc3fcb42c84a85db531b3fb9a227d4fa5432799aba0d86b478
...
class TrustedWorkflowPolicyTest(unittest.TestCase):
  def assert_current_main_baseline_is_exact(self, policy):
    ...
    self.assertEqual(temporary["id"], "temporary-gatt-stack-df2ac48")
    self.assertEqual(temporary["mode"], "temporary-exact")
    self.assertEqual(
        persistent["id"],
        "future-gatt-stack-df2ac48-persistent-baseline",
    )
  ...
  def test_transition_has_exact_and_future_persistent_baselines(self):
    ...
    self.assertEqual(bundle["id"], "temporary-gatt-stack-df2ac48")
    ancestry.assert_not_called()
    self.assertEqual(
